...
void print_info(char *host, char *port, char *nexthost, char *nextport) {
    printf("----------------------------\n"
           "Domain name: %s\n"
           "Listening to port: %s\n"
           "Sending to host: %s\n"
...
int main(int argc, const char *argv[]) {
    ...
    print_info(domain, options->localport, options->nexthost, options->nextport);
    ...
    pthread_t threads[2];
    pthread_create(&threads[0], NULL, &tcp_server, (void *) options);
    pthread_create(&threads[1], NULL, &tcp_client, (void *) options);
