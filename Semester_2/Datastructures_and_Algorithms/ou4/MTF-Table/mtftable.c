...
//(And a bit of the table_inspect is written by Jonas Sjödin <id16jsn@cs.umu.se>)
...
/*
 * Searches for a specific key and if it is found it moves that specific
 * element to the front of the table.
 *
 * table - Pointer to the table. 
 * key - Pointer to the key thats about to be searched for. 
 */
VALUE table_lookup(Table *table, KEY key) {
    ...
    while (!dlist_isEnd(t->values,p)) {
        ...
        if (t->cf(i->key,key)==0){
					...
					dlist_insert(t->values, dlist_first(t->values),element);
